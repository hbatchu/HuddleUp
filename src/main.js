...
function createGame(category) {
  socket.emit('game:create', { category }, result => {
    if (!result.ok) return;
...
function host() {
  ...
  if (g.phase === 'lobby') { const link = `${location.origin}/?pin=${g.code}`; return `<main class="room host-room"><header>${logo()}<span class="status"><i></i> LIVE ROOM</span></header><section class="lobby"><div class="eyebrow">${g.title.toUpperCase()}</div><div class="pin-label">GAME PIN</div><div class="big-pin">${g.code}</div><p>Share the room link or have players enter the PIN.</p><button class="share-link" id="copy-link" data-link="${link}"><span>↗</span><b>${link.replace(/^https?:\/\//, '')}</b><em>Copy link</em></button><div class="people player-list">${g.players.length ? g.players.map((p,i)=>`<span class="player-chip"><i class="av a${i % 5}">${playerInitial(p.name)}</i><b>${safeText(p.name)}</b></span>`).join('') : '<span class="empty-players">Waiting for players to join…</span>'}</div><b class="player-total">${g.players.length} player${g.players.length === 1 ? '' : 's'} in the room</b><button class="primary massive" id="start" ${g.players.length ? '' : 'disabled'}>Start the game <span>→</span></button><small class="hint">${g.players.length ? 'Everyone is ready. Let’s go!' : 'Waiting for your first player…'}</small></section></main>`; }
  if (g.phase === 'question') return `<main class="room host-room"><header>${logo()}<span class="round">QUESTION ${g.questionIndex + 1} / ${g.questionCount}</span></header><section class="host-question"><div class="q-meta"><span class="q-pill">${q.level.toUpperCase()} · ${g.title.toUpperCase()}</span><span class="answer-progress"><b>${count}</b> / ${g.players.length} ANSWERED <em>· ${Math.max(0, g.players.length - count)} ANSWERING</em></span></div><h2>${q.question}</h2>${q.image ? `<img class="question-image" src="${q.image}" alt="Visual cue for the question">` : ''}<div class="answer-grid mini">${q.answers.map((a,i)=>`<div class="answer ${q.colors[i]}"><b>${icons[i]}</b>${a}</div>`).join('')}</div>${reactionShelf(false)}<button class="primary reveal" id="reveal">Reveal answers <span>→</span></button></section></main>`;
  return results(true);
}
function playerLobby() { return `<main class="room player-room"><header>${logo()}<span class="status"><i></i> CONNECTED</span></header><section class="waiting"><div class="waiting-icon">✦</div><div class="eyebrow">YOU’RE IN!</div><h2>Hey, ${safeText(state.player.name)}.</h2><p>Get comfortable — the host will start the game any moment.</p><div class="game-chip"><span>${state.game.title}</span><b>PIN ${state.game.code}</b></div><div class="pulse-row"><i></i><i></i><i></i></div></section></main>`; }
function playerGame() {
  const g = state.game;
  ...
  return `<main class="room player-room"><header>${logo()}<span class="round">${g.questionIndex + 1} / ${g.questionCount}</span><span class="score">${state.player.score} pts</span></header><section class="question"><div class="timer ${state.timeLeft <= 5 ? 'urgent' : ''}"><span>${state.answering ? '✓' : state.timeLeft}</span></div><p class="question-count">QUESTION ${g.questionIndex + 1} · ${state.timeLeft ? 'ANSWER FAST' : 'TIME IS UP'}</p><h2 class="phone-prompt">${q.question}</h2><p class="answer-instruction">Choose the color of the right answer</p><div class="answer-grid player-answers">${q.answers.map((_,i)=>`<button aria-label="Choose ${q.colors[i]} option" class="answer ${q.colors[i]} ${state.selected === i ? 'picked' : ''}" data-answer="${i}" ${state.answering || state.timeLeft <= 0 ? 'disabled' : ''}><b>${icons[i]}</b></button>`).join('')}</div>${state.answering ? `<p class="answered">Answer locked in — nice and quick!</p>` : state.timeLeft ? '<p class="choose">Match the color on the host screen</p>' : '<p class="choose">Waiting for the host to reveal the answer</p>'}${reactionShelf()}</section></main>`;
}
...
function results(isHost) {
  ...
  return `<main class="room results ${done ? 'final-results' : ''}"><header>${logo()}<span class="status"><i></i> ${done ? 'GAME COMPLETE' : 'ROUND RESULTS'}</span></header><section><div class="celebrate">${done ? '🏆' : '⚡'}</div><div class="eyebrow">${done ? 'FINAL LEADERBOARD' : 'SCORES UPDATED'}</div><h2>${done ? 'That was a huddle!' : 'Here’s how it stands'}</h2>${feedback}${standings}${isHost ? `<button class="primary massive" id="${done ? 'again' : 'next'}">${done ? 'Play again' : 'Next question'} <span>→</span></button>` : `<p class="wait-next">${done ? 'Thanks for playing HuddleUp.' : 'Waiting for the host to continue…'}</p>`}</section></main>`;
}
function bind() {
